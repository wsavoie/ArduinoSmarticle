...
for i=1:nMovs
    ...
    try
        for jj=1:numBods
            [movs(idx).t(:,jj),movs(idx).x(:,jj),movs(idx).y(:,jj),~,fps,movs(idx).rot(:,jj)]= trackOptitrack(fullfile(fold,f(i).name),dec,[' ',num2str(jj)]);
        end
        if exist('frameName','var')
            ...
            [~,movs(idx).FRx,movs(idx).FRy,movs(idx).FRdata,movs(idx).fps, movs(idx).FRrot]= trackOptitrack(fullfile(fold,f(i).name),dec,frameName);
        end
