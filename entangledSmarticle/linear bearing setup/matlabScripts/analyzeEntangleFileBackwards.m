...
%     dsPts(1,3)=1;
    [~,d]=findpeaks(mys,'npeaks',fpars(6)/2,'minpeakdistance',length(t)/fpars(6)*peakdisk);
    dsPts(1:4:np,3)=d;
...
%     else
  startInd=2000;
    [~,c]=findpeaks(mye(startInd:end),'npeaks',fpars(6)/2,'minpeakdistance',length(t)/fpars(6));
