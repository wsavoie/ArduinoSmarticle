...
fold=uigetdir('A:\Dropbox\smartmovies\pavel-smartVariousVidsAndPics\PavelTrip2\interrupt_3smarts\Optitrack_CSV');
...
if RIGIDBODYNAMES
    ...
    warning(['make sure to correct "numBods" to the number of '...
        'smarticles in the regStates data taken current set at numBods=',num2str(numBods)]);
...
for i=1:nMovs
    ...
    if RIGIDBODYNAMES
        for jj=1:numBods
            [movs(idx).t(:,jj),movs(idx).x(:,jj),movs(idx).y(:,jj),~,fps,movs(idx).rot(:,jj)]= regStatesOptitrack(fullfile(fold,f(i).name),dec,[' ',num2str(jj)]);
        end
...
%         r(qm)=r(qm)+2*pi;
        ...
        %                 for jj=1:cdnumBods
        [movs(idx).t,movs(idx).x,movs(idx).y,~] = regStatesOptitrack(fullfile(fold,f(i).name),dec,'');
        %             end
